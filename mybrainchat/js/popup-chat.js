/**
 * Popup chat functionality for mybrainchat plugin
...
function initPopupChat(courseId) {
    ...
    const chatButton = document.getElementById('mybrainchat-open-button');
    if (chatButton) {
...
function createModalStructure() {
    ...
    modalOverlay.className = 'mybrainchat-modal-overlay';
    modalOverlay.id = 'mybrainchat-modal-overlay';
    
    const modalContainer = document.createElement('div');
    modalContainer.className = 'mybrainchat-modal-container';
    
    const modalClose = document.createElement('button');
    modalClose.className = 'mybrainchat-modal-close';
    modalClose.innerHTML = '&times;';
    modalClose.setAttribute('aria-label', 'Close chat');
    modalClose.addEventListener('click', closeModal);
    
    const modalContent = document.createElement('iframe');
    modalContent.className = 'mybrainchat-modal-content';
    modalContent.id = 'mybrainchat-modal-content';
...
function openChatPopup(courseId) {
    ...
    // Find the first available mybrainchat instance in the course
    // This is done via AJAX to avoid having to pass all instance data to the page
    fetch(baseUrl + '/mod/mybrainchat/api/get_instance.php?courseid=' + courseId)
        .then(response => response.json())
        .then(data => {
            if (data.success && data.cmid) {
                // Get the modal elements
                const modalOverlay = document.getElementById('mybrainchat-modal-overlay');
                const modalContent = document.getElementById('mybrainchat-modal-content');
                ...
                const chatUrl = baseUrl + '/mod/mybrainchat/view.php?id=' + data.cmid + '&popup=1';
                modalContent.src = chatUrl;
                
                // Show the modal
                showModal();
            } else {
                console.error('No mybrainchat instance found for this course');
                alert('Kein Chat für diesen Kurs verfügbar.');
            }
        })
        .catch(error => {
            console.error('Error fetching mybrainchat instance:', error);
