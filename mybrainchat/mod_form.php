...
/**
 * The main mod_mybrainchat configuration form.
 *
 ...
 * @license     https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_mybrainchat_mod_form extends moodleform_mod {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'name', get_string('mybrainchatname', 'mod_mybrainchat'), ['size' => '64']);
        ...
        $mform->addHelpButton('name', 'mybrainchatname', 'mod_mybrainchat');
        ...
        // Adding the rest of mod_mybrainchat settings, spreading all them into this fieldset
        $mform->addElement('static', 'label1', 'mybrainchatsettings', get_string('mybrainchatsettings', 'mod_mybrainchat'));
        $mform->addElement('header', 'mybrainchatfieldset', get_string('mybrainchatfieldset', 'mod_mybrainchat'));
        ...
        if ($has_saved_key) {
            ...
            $mform->addElement('static', 'apikey_info', '',
                ...
                get_string('apikey_from_profile_desc', 'mod_mybrainchat') .
                '</div>'
            );
        }

        // API Key (password field) - optional if user has saved key
        $mform->addElement('passwordunmask', 'apikey', get_string('apikey', 'mod_mybrainchat'));
        ...
        $mform->addHelpButton('apikey', 'apikey', 'mod_mybrainchat');

        // Button to load brains
        $mform->addElement('button', 'loadbrains', get_string('loadbrains', 'mod_mybrainchat'), [
            'id' => 'id_loadbrains'
        ]);
        ...
        $brainoptions = ['' => get_string('selectbrain', 'mod_mybrainchat')];
        ...
        $mform->addElement('select', 'brainid_select', get_string('brainid', 'mod_mybrainchat'), $brainoptions, [
            'id' => 'id_brainid_select'
        ]);
        $mform->addHelpButton('brainid_select', 'brainid', 'mod_mybrainchat');

        // Status message area
        $mform->addElement('static', 'brains_status', '', '<div id="brains_status"></div>');

        // Popup settings header
        $mform->addElement('header', 'popupsettings', get_string('popupsettings', 'mod_mybrainchat'));

        // Use for popup checkbox
        $mform->addElement('advcheckbox', 'use_for_popup', get_string('use_for_popup', 'mod_mybrainchat'),
            get_string('use_for_popup_desc', 'mod_mybrainchat'), [], [0, 1]);
        $mform->setDefault('use_for_popup', 0);
        $mform->addHelpButton('use_for_popup', 'use_for_popup', 'mod_mybrainchat');
        ...
        $PAGE->requires->js_amd_inline("
            require(['jquery'], function(\$) {
                ...
                \$('#id_loadbrains').on('click', function() {
                    ...
                    statusDiv.html('<div class=\"alert alert-info\">" . get_string('loadingbrains', 'mod_mybrainchat') . "</div>');

                    \$.ajax({
                        url: wwwroot + '/mod/mybrainchat/api/get_brains.php',
                        method: 'GET',
                        data: { apikey: apikey },
                        dataType: 'json',
                        success: function(response) {
                            if (response.success && response.brains.length > 0) {
                                brainSelect.empty();
                                brainSelect.append('<option value=\"\">" . get_string('selectbrain', 'mod_mybrainchat') . "</option>');
                                ...
                                statusDiv.html('<div class=\"alert alert-success\">" . get_string('brainsloaded', 'mod_mybrainchat') . " (' + response.brains.length + ')</div>');
                            } else {
                                statusDiv.html('<div class=\"alert alert-warning\">' + (response.error || '" . get_string('nobrainsfound', 'mod_mybrainchat') . "') + '</div>');
                            }
                        },
                        error: function(xhr, status, error) {
                            statusDiv.html('<div class=\"alert alert-danger\">" . get_string('errorloadingbrains', 'mod_mybrainchat') . ": ' + error + '</div>');
                        }
                    });
                });
